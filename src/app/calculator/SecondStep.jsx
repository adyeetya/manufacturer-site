  ...
  const renderedSpaces = spaces.map((space) => {
    ...
    if (defaultCount > 0) {
      return (
        <div
          key={space.name}
          className="flex items-center justify-between h-20 mb-4 bg-white rounded p-2 mx-4"
        >
  ...
  const unrenderedSpaces = spaces
    ...
    .map((space) => (
      <button
        ...
        className="bg-blue-500 text-sm text-white px-4 py-2 rounded hover:bg-blue-700 m-2"
      >
        {space.name}
      </button>
    ))
